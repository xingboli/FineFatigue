...
export const StarCatcherGame: React.FC<StarCatcherGameProps> = ({ onGameEnd }) => {
  ...
  const start = () => {
    ...
    setBounds({ width, height }); startedAtRef.current = Date.now(); playerRef.current = { x: width / 2, y: height * 0.84 }; pathRef.current = []; insideNoGoRef.current = false; noGoEntriesRef.current = 0; noGoDwellRef.current = 0; noGoStartedAtRef.current = null; setTimeLeft(25); setResult(null); setPlaying(true);
  };
