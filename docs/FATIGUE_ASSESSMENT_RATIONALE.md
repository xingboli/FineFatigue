...
FineFatigue 的固定轨迹追踪任务要求受试者连续跟随动态目标，观察持续的视觉—运动闭环控制。

系统记录 tracking RMSE、在靶比例和相位滞后，从而形成连续性能监测通道。当前任务不设置禁区或 No-Go 抑制要求。
...
本节将前述理论依据与当前代码实现对齐，避免把“任务设计有理论来源”误写成“FineFatigue 的整套电池已经完成临床验证”。

- Full / Research Mode 的当前代码参数来自 `src/config/runtime.ts`：校准 3 s、稳定性基线和后测各 15 s、敲击基线和后测各 15 s、反应时需要 30 个有效试次（Full 随机等待 2–10 s）、疲劳挑战 30/60 s、Star Catcher 25 s。Demo Mode 使用 1.5/5/8 s、5 个反应时有效试次、0.7–1.8 s 随机等待、10/20 s 挑战和 10 s Star Catcher；这些缩短参数适合课堂、公开演示和流程检查，不应作为正式研究协议的等价数据。
- 当前敲击分段按实际录制时长的三个等长区间计算；结果类型仍保留 `first5sRate`、`middle5sRate`、`last5sRate` 等历史字段名，因此字段名不应被解读为 Full 一定采用固定 5 s 分段。
- 原始测量、原始派生值和展示值有区分。例如稳定性分数、描摹 RMSE/平滑度和认知分数存在展示范围裁剪；研究分析应优先使用原始字段，并同时记录版本和运行模式。
- 当前疲劳指数使用稳定性 25%、运动耐力 30%、反应能力 20%、精细运动控制 25%，并使用代码中的等级阈值和范围裁剪。这些是透明的工程启发式参数，不是临床常模，也尚未由 FineFatigue 数据验证。
- 空间记忆任务和 Star Catcher 是独立保存/同步/导出的任务结果；它们目前不进入主评测报告的四维疲劳指数。它们可以补充研究变量，但不能被写成主指数已经综合了认知或连续追踪能力。
- 代码确实读取真实浏览器 IMU 事件，不生成模拟 IMU fallback；这证明了输入来源是真实传感器事件，但不等于已经证明跨设备测量等价、构念效度或临床效度。

因此，本文献与理论部分支持的是任务构念选择、行为指标解释和前后测设计逻辑；关于 FineFatigue 特定分数、权重、阈值、跨人群泛化和疾病识别能力，仍应以本项目后续的预注册协议、质量控制、统计分析和外部验证为依据。
