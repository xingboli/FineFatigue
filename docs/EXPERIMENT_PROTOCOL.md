...
Demo 使用真实交互，但参数缩短：校准 1.5 s、稳定性 5 s、敲击 8 s、反应 5 个有效试次且等待 0.7–1.8 s、挑战 10/20 s、Star Catcher 10 s。Demo 无真实 Full 账户、云同步、服务端 API 或 CSV 导出；其一键演示身份切换以及只读示例历史/管理员数据仅服务于 UI 演示。它用于演示和流程 smoke check，不用于与 Full 数据直接合并的正式统计分析。
