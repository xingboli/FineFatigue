...
Online Demo 是同一代码库的纯静态 Demo 构建，面向课堂、展示和快速体验。它不需要 Node.js、Express、数据库或任何后端 API；实验交互仍使用真实的浏览器点击、Canvas 和设备传感器数据，新增记录只保存在当前浏览器的 LocalStorage。Demo 会缩短计时以便快速体验，不会生成虚假 IMU；它会在运行时展示明确标注的只读示例历史和账号数据，供演示趋势与权限界面，不会写入 LocalStorage、同步或导出。
...
| 真实用户注册/登录 | ❌；可一键切换演示受试者/管理员视图 | ✅ |
| 云同步与数据库 | ❌ | ✅ |
| 管理员后台与 CSV 汇总 | 只读示例界面；无 API/导出 | ✅ |
...
- 中英文界面与 Star Catcher 独立标准化追踪任务：固定高难度李萨如轨迹与时长；以 RAF 固定约 30 Hz 写入原始指针/目标点，即使指针静止仍采样，结束后展示完整双轨迹、RMSE、在靶时间和相位滞后。为数据兼容保留的旧禁区字段恒为零且不在 UI 展示。
- 完成反馈：敲击、稳定性、反应、疲劳挑战、主观自评、认知任务和 Star Catcher 在完成时调用同一段短暂、可尊重系统“减少动态效果”设置的庆祝反馈；螺旋描摹不触发该反馈。
