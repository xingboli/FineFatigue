...
- 认知与记忆测试：提供独立的 4×4 空间记忆配对任务，使用本地静态符号随机排布；记录真实点击、配对尝试、正确/错误、响应时间和前后半程变化，不生成虚构受试者记录，也不作医学诊断。
- 账号与同步：受试者可注册密码账号；新账号需要管理员批准后才能登录。已登录受试者可通过同源 LAN 服务同步浏览器中的会话、自评和设置。
- 管理员端：受试者审核/停用、参与次数查看、密码重置、报酬金额与状态记录，以及会话、受试者和认知测试汇总 CSV 导出。
...
- 浏览器 LocalStorage 也会缓存当前设备上的会话、自评、认知任务结果、设置和登录令牌。清除浏览器站点数据会删除这些本地缓存。
- 认知测试的完整本地/同步记录包含交互事件和配对尝试，管理员认知 CSV 仅导出便于训练的汇总指标。所有导出仍属于研究数据，应按实验伦理与数据管理要求保留。
