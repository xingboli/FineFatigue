...
- 标准评测流程：传感器校验、4 项基准测验、重复运动负荷挑战、4 项负荷后复测、主观疲劳自评与报告生成。
- 4 个客观维度：手部稳定性、运动耐力/敲击、视觉反应、螺旋描摹精细控制。主观自评被保存为独立多维实验变量，不参与客观疲劳指数计算。
- 会话管理：本地保存会话、历史回看、报告 JSON 导出和浏览器打印。
- 账号与同步：受试者可注册密码账号；新账号需要管理员批准后才能登录。已登录受试者可通过同源 LAN 服务同步浏览器中的会话、自评和设置。
- 管理员端：受试者审核/停用、参与次数查看、密码重置、报酬金额与状态记录，以及会话/受试者 CSV 导出。
- 中英文界面与 Star Catcher 独立小游戏。
...
.
├─ src/
│  ├─ components/       # 评测步骤、图表、通用组件、布局、小游戏
│  ├─ pages/            # 概览、传感器监视、报告、历史、设置、管理员页
│  ├─ services/         # IMU、认证、同步、本地存储、AI 建议
│  ├─ utils/            # 信号处理与疲劳评分
│  ├─ types/            # 前后端共用的核心数据类型
│  └─ i18n/             # 中英文文案与语言上下文
├─ public/              # 静态资源
...
- 服务端会在 `data/finefatigue-store.json` 保存受试者账户、密码哈希、会话、自评、设置和报酬管理记录；该目录被 `.gitignore` 排除。
- 浏览器 LocalStorage 也会缓存当前设备上的会话、自评、设置和登录令牌。清除浏览器站点数据会删除这些本地缓存。
- 管理员 CSV 使用受试者编号而非姓名/邮箱，但仍属于研究数据；导出、传输和保留应遵循实验伦理与数据管理要求。
